...
type ContactServiceIface interface {
	...
	SendMessage(ctx context.Context, peerID peer.ID, msgType string, mimeType string, payload []byte, attachments []string) (resultCh <-chan mytype.ContactMessage, err error)
